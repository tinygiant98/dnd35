...
void main()
{
    ...
    if(GetLocalInt(oMod,"MULTICHAR"))
    {
        sReport+="\n- MULTICHAR is set at "+
            IntToString(GetLocalInt(oMod,"MULTICHAR"))+".";
    }
    sReport += "\n- DECONENTER is ";
    ...
    if(GetLocalInt(oMod,"HCREXP"))
    {
        sReport+="on with a BASEXP setting of "+
            ...
            IntToString(GetLocalInt(oMod,"BONUSXP"))+". PWEXP is ";
    }
    else sReport +="off. PWEXP is ";
    ...
    if(GetLocalInt(oMod,"LOOTSYSTEM"))
        sReport+="on. DYINGSTRIP is";
    else
        sReport+="off. DYINGSTRIP is ";
    ...
    if(GetLocalInt(oMod,"UNIDONDROP"))
        sReport+="on, NPCCORPSE is ";
    else
        sReport+="off, NPCCORPSE is ";
    if(GetLocalInt(oMod,"NPCCORPSE"))
        sReport+="on, SUBRACES are ";
    else
        sReport+="off, SUBRACES is ";
    if(GetLocalInt(oMod,"SUBRACES"))
        sReport+="on, PWDBSAVE is ";
    else
        sReport+="off, PWDBSAVE is ";

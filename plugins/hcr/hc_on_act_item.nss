// HCR 3.03b added support for Old Man Whistlers Loot Notification - cfx

// hcr3
// added heal ability token code.
// took out hello conversation.
// sr6.0
// added code to not check for npcs on healkit code.
// sr 5.4.1
// added DMFI support for enhance DM support.
// sr5.4
// added code to use stackable potions use scure at end of tag name.
// example "PotionOfCLWscure".
// sr5
...
void main()
{
    ...
    {
        if (GetLocalInt(GetModule(), "DIRTRACK"))
           ExecuteScript("hc_track_start", oUser);
        if (GetLocalInt(GetModule(), "PLACETRACK"))
           ExecuteScript("hc_track_start2", oUser);
        postEvent();
        return;
    }
...
// Thieves' Tools
   ...
   //Begin Healer's Kit = With thanks to Andalus for taking the time to make.
    ...
    {
        object oOther=GetItemActivator();
        SetLocalObject(oUser,"OTHER",oOther);
        SetLocalObject(oUser,"ITEM",oItem);
        SetLocalString(oUser,"TAG",sItemTag);
    ...
    //DMFI support
    if (GetStringLeft(sItemTag,8) == "hlslang_"||GetStringLeft(sItemTag, 5) == "dmfi_")
    {
...
// PC hasnt got their hands on it.
   ...
   if(sHTarget!="")
   {
      ...
      while (GetIsObjectValid(oTest) == TRUE)
       {
        if (GetPCPlayerName(oTest) == sTestName && GetIsDM(oTest) == FALSE &&
            GetPCPlayerName(oTest)!=GetPersistentString(oMod,"PLAYERDM"))
            {
            DestroyObject(oItem);
      ...
      //hcr3
      // took out hello sound.
      AssignCommand(oDM, ActionStartConversation(oDM, sHConv, TRUE, FALSE));
      postEvent();
      return;
   }
